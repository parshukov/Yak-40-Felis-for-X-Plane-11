...
-- pressure value
defineProperty("pressure", globalPropertyf("sim/custom/xap/gauges/vd10ft_press"))
...
function update()
	...
	local alt = real_alt-- * 0.3048 -- altitude in feet
	--if alt > 10000 then alt = 10000 end
	
	ten_thousand_angle = alt * 0.00001 * 360.0
	...
	pressure_num = virt_press * 33.863726
...
--]]	
    ...
    -- ten thousands meters needle
    needle {
        position = { 20, 20, 160, 160 },
        image = get(bigNeedleImage),
        angle = function() 
            return ten_thousand_angle
        end
    }, 
